...
export default function PatientTopNav() {
  ...
  const [welcomeName, setWelcomeName] = useState("Patient");
  ...
  useEffect(() => {
    async function fetchPatientName() {
      try {
        ...
        if (loggedInUser.role !== "PATIENT" || !loggedInUser.patientId) return;

        const response = await fetch(
          `/api/patient-sidebar/${loggedInUser.patientId}`,
        );

        const data: PatientSidebarResponse = await response.json();

        if (!response.ok) return;

        setWelcomeName(data.patient.firstName);
      } catch (error) {
        console.error("Error loading patient top nav:", error);
      }
    }
      ...
      <nav className="bg-blue-300 py-4">
        <div className="row">
          <div className="flex h-16 w-full items-center justify-between">
            ...
            >
              <span className="sr-only">Open patient menu</span>
              <svg
          ...
          {isMenuOpen && (
            <div id="patient-mobile-menu" className="mt-3 lg:hidden">
              ...
              <div className="overflow-hidden rounded-xl bg-teal-100 shadow-lg">
                <PatientSideNav isMobile onClose={closeMenu} />
              </div>
            </div>
          )}
        </div>
